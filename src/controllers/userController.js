    ...
    if (data) {
        if (data == 1) {
            const [point_lists] = await connection.query('SELECT * FROM point_list WHERE `phone` = ? ', [rows[0].phone]);
               //this if condition by harsh
             if (point_lists.length === 0) {
        return res.status(404).json({
            message: 'Point list not found',
            status: false,
            timeStamp: timeNow,
        });
    }
...
const useRedenvelope = async (req, res) => {
    ...
    if (!auth || !code) {
     ...
     if (emojiRegex.test(code)) {
        return res.status(400).json({
            message: 'Invalid code: Emojis are not allowed',
            status: false,
            timeStamp: timeNow,
        });
    }
    ...
    if (redenvelopes.length == 0) {
        ...
        console.log(redenvelopes[0],"redenvelopes");
